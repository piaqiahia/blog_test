...
var refreshTab = function(currentId, no_exist_callback){
    var topWindow = $(window.parent.document);
    if(!currentId || currentId.length == '')
        currentId = $('.page-tabs-content', topWindow).find('.active').attr('data-id');
    var target = $('.RuoYi_iframe[data-id="' + currentId + '"]', topWindow);
    var url = target.attr('src');
    console.log(url)
    if(!target || !url || url != currentId) {
        if(no_exist_callback) {
            no_exist_callback(currentId)
        }
        return;
    }
    target.attr('src', url).ready();
...
var sub = {
    ...
    delRow: function(column, tableId) {
...
$.ajaxSetup({
    ...
    complete: function(XMLHttpRequest, textStatus) {
        if (textStatus == 'timeout') {
            ...
            && XMLHttpRequest.responseJSON.code == 400 
            && XMLHttpRequest.responseJSON.msg.indexOf('Refresh retry again')){
            window.top.$.modal.confirm("由于长时间未操作，已与服务器断开，请刷新重新连接", function() {
                window.top.$.modal.reload();
            });
