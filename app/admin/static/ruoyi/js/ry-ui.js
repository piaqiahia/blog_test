    ...
    $.extend({
    	...
    	table: {
            	...
            	$(optionsIds).off("click").on("click", '.img-circle', function() {
                    ...
                    function scale(maxW, maxH, orgW, orgH) {
                        if(orgW/orgH >= maxW/maxH) {
                            if(orgW > maxW) {
                                return { w: maxW, h : (orgH * maxW) / orgW }
                            }else {
                                return { w: orgW, h: orgH }
                            }
                        }else {
                            if(orgH > maxH) {
                                return {w: (orgW * maxH) / orgH, h: maxH}
                            }else {
                                return {w: orgW, h: orgH}
                            }
                        }
                    ...
                    getImageWidth(url, function (w, h) {
                        console.log("win:" + windowW + "," + windowH);
                        console.log("img:" + w + "," + h);
                        // 调整图片大小
                        if (w > windowW || h > windowH) {
                            if (w > windowW && h > windowH) {
                                // w = windowW * 0.75;
                                // h = windowH * 0.75;
                                let sc = scale(windowW, windowH, w, h)
                                console.log(sc)
                                w = sc.w
                                h = sc.h
            ...
            exportExcel: function(formId) {
                ...
                $.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function() {
                    ...
                    var ids = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId); //可选择导出excle
                    dataParam.push({ "name": "ids", "value": ids}); //可选择导出excle
                    $.modal.loading("正在导出数据，请稍候...");
                    $.post(table.options.exportUrl, dataParam, function(result) {
                        if (result.code == web_status.SUCCESS) {
                            window.location.href = "/admin/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
            ...
            exportExcel2: function(formId) {
                ...
                $.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function() {
                    var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                    var params = $("#" + table.options.id).bootstrapTable('getOptions');
                    var dataParam = $("#" + currentId).serialize();
                    dataParam += '&orderByColumn=' + params.sortName;
                    dataParam += '&isAsc=' + params.sortOrder;

                    let download_url = table.options.exportUrl + '?' + dataParam;
        ...
        operate: {
            ...
            add: function(id,title) {
            	table.set();
                title = $.common.isEmpty(title) ? "添加" + table.options.modalName : title;
            ...
            addTab: function (id,title) {
            	table.set();
                title = $.common.isEmpty(title) ? "添加" + table.options.modalName : title;
                $.modal.openTab(title, $.operate.addUrl(id));
            },
            // 添加信息 全屏
            addFull: function(id,title) {
            	table.set();
                title = $.common.isEmpty(title) ? "添加" + table.options.modalName : title;
                $.modal.openFull(title, $.operate.addUrl(id));
            },
            // 添加访问地址
            addUrl: function(id) {
            ...
            edit: function(id,title) {
            	table.set();
                title = $.common.isEmpty(title) ? "修改" + table.options.modalName : title;
