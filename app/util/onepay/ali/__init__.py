...
class AliPay(object):
    ...
    def __init__(self, config):
        ...
        self._app_private_key_path = config.get('private_key_path','')
        self._app_public_key_path = config.get('public_key_path','')
        self._sign_type = config.setdefault('sign_type', 'RSA2')
        self.debug = config.setdefault('debug', False)
        self._app_private_key = config['private_key']
        self._alipay_public_key = config['public_key']
    ...
    def _load_key(self):
        ...
        if self._app_private_key_path != '':
            ...
            self._app_private_key = RSA.importKey(pstr.format(self._app_private_key))

        # load public key
        if self._app_public_key_path != '':
            with open(self._app_public_key_path) as fp:
                content = fp.read()
            self._alipay_public_key = RSA.importKey(content)
        else:
            self._alipay_public_key = RSA.importKey(pstr.format(self._alipay_public_key))
    ...
    def trade_precreate(self, return_url=None, **kwargs):
        """二维码支付"""
        ...
        resp = requests.get(self._gateway + '?' + self.sign_data(data))
        result = resp.json()
        if result['alipay_trade_precreate_response'] and result['alipay_trade_precreate_response']['msg'] == 'Success':
            ...
            return f"data:image/jpeg;base64,{base64_data.decode('ascii')}"
        return ''
