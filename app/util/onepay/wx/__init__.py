...
class WxPay(object):
    ...
    def _sign(self, raw):
        ...
        """
        raw = [(k, str(raw[k]) if isinstance(raw[k], (int, float)) else raw[k]) for k in sorted(raw.keys(),reverse=False)]
        s = '&'.join('='.join(kv) for kv in raw if kv[1])
        s += '&key={0}'.format(self._mch_key)
        md5 = hashlib.md5()  # 使用MD5加密模式
        md5.update(s.encode('utf-8'))  # 将参数字符串传入
    ...
    def _fetch(self, url, data):
        try:
            # print(data)
            # print('>>>>>== ',dict_to_xml(data))
            resp = requests.post(url, data=dict_to_xml(data).encode('utf-8')).content
            # print(resp)
    ...
    def trade_precreate(self, return_url=None, **kwargs):
        """二维码支付"""
        qrcode_url = self.trade_page_pay(**kwargs)
